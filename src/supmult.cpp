...
public:
  ...
  BinSpMat(Rcpp::S4 mat, double scale = 1, int nthread = 1) :
  ...
  // res = A * v
  void prod(const double* v, double* res, double scale = 1.0) const
  {
    if(m_compute_transpose)
    {
      ...
      for(int i = 0; i < m_n; i++)
      {
        ...
        res[i] = gather_sum(v, Aj_start, outer[i + 1] - outer[i]) / scale;
      }
      return;
    }
  ...
  // res = A' * v
  void tprod(const double* v, double* res, double scale = 1.0) const
  {
    const int* inner = m_sp.innerIndexPtr();
    const int* outer = m_sp.outerIndexPtr();
    for(int j = 0; j < m_n; j++)
    {
      const int* Ai_start = inner + outer[j];
      res[j] = gather_sum(v, Ai_start, outer[j + 1] - outer[j]) / scale;
    }
  }
};
...
// res = [(AA')P, ..., (AA')^qP]
// [[Rcpp::export]]
NumericMatrix krylov_space_part(SEXP coords, NumericMatrix P, double scale = 1.0, int q = 1, int nthread = 1)
{
  Rcpp::XPtr<BinSpMat> sp(coords);
  const int n = P.nrow();
  const int k = P.ncol();
  NumericMatrix res(Rcpp::no_init_matrix(n, q * k));
...
#pragma omp parallel for shared(P_ptr, res_ptr, sp) num_threads(nthread)
  for(int j = 0; j < k; j++)
  {
    // Allocate workspace for the intermediate result
    ...
    // Compute A' * P[, j], store in work
    sp->tprod(v, work, scale);
    // Compute A * (A' * P[, j]), store in res[, j]
    sp->prod(work, r, scale);

    // Power iterations for (AA')^2P, ..., (AA')^qP
    // (AA')^iP [, j] = (AA')(AA')^(i-1)P [, j]
    for(int i = 1; i < q; i++)
    {
      const double* rin = r + (i - 1) * k * n ;
      double* rout = r + i * k * n;
      sp->tprod(rin, work, scale);
      sp->prod(work, rout, scale);
    }
    // Free workspace
    delete[] work;
  }
